...
import '../dto/upload_file_dto.dart';
...
class ApiProvider {
  ...
  Stream<Either<CustomException, UploadFileDto>> postMultiPartStream(
      String endPoint, FormData body, String filePath,
      {String? authToken}) async* {
    try {
      StreamController<Either<CustomException, UploadFileDto>> controller =
          StreamController();

      StreamController<UploadFileDto> progressStream = StreamController();
      if (authToken != null) {
        dio.options.headers["authorization"] = "Bearer $authToken";
      }
      dio.post(
        ...
        onSendProgress: (int count, int total) {
          progressStream.add(UploadFileDto(progress: (count / total)));
        },
      ).then((value) async {
        if ([200, 201].contains(value.statusCode)) {
          progressStream.add(
              UploadFileDto(progress: 100, response: classifyResponse(value)));
        } else {
          progressStream.add(UploadFileDto(
              error: true, errorMessage: classifyErrorDownloadResponse(value)));
        }
        progressStream.close();
      }, onError: (error) async {
        progressStream.add(UploadFileDto(
            progress: 100, error: true, errorMessage: "Kesalahan unggah file"));
      });

      progressStream.stream.listen((event) {
        if (event.error) {
          controller.add(Left(ServerException(event.errorMessage)));
          progressStream.close();
        } else {
      ...
      yield Left(ConnectionException());
    }
  }
